...
int create_window(Window* window, int width, int height, char* title) {
  ...
  if(extensions != 0) {
    printf("Found glExtensions:\n");
    int printed = 0;
    int offset = 0;
    do {
      printf("\t");
      printed = print_until_space(extensions + offset);
      printf("\n");
      offset += printed + 1;
    } while(printed > 0);
  }
  */
  ...
  printf("Found wglExtensions:\n");
  int printed;
  int offset = 0;
  do {
    printf("\t");
    printed = print_until_space(wgl_extensions + offset);
    printf("\n");
    offset += printed + 1;
  } while(printed > 0);
